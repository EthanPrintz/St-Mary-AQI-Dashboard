...
function App() {
  ...
  let graph_names = ['PM 25', 'PM 10', 'Temperature', 'Humidity'];
    ...
    async function updateSensorData() {
		const BASE_URL = 'https://u50g7n0cbj.execute-api.us-east-1.amazonaws.com/v2/measurements?format=json&';
		...
		const selectedAirFactors = graph_names.map((item) => item.toLowerCase().replace(/\s/g, ''));;
		...
		response.then((raw)=>{
			...
			//pm25
			let pm25 = data.filter((item) => item.parameter == "pm25").map((item) => 10* item.value)
			//pm10
			let pm10 = data.filter((item) => item.parameter == "pm10").map((item) => 10* item.value)
